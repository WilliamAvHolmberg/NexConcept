...
import org.nexus.communication.message.respond.QuestRespond;
...
public class RequestTask extends NexRequest{
	...
	public void execute(PrintWriter out, BufferedReader in) throws IOException {
		...
		for(Quest quest : Quest.values()) {
			if(!methodProvider.getQuests().isMembers(quest)) {
			quests += quest + "," + methodProvider.getQuests().isComplete(quest) + ";";
			}
		}
		String request = "task_request:1:" + string + ":" + quests;
		methodProvider.log(request);
		out.println(request);
	...
	private void handleRespond(String respond) {
		if (respond.contains("TASK_RESPOND:0") || respond.contains("DISCONNECT")) {
			...
			case "AGILITY":
				...
				break;
			case "QUEST":
				messageQueue.push(new QuestRespond(methodProvider, messageQueue, respond));
