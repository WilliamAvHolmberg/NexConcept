...
public final class LoginEvent extends Event {
	...
	public final int execute() throws InterruptedException {
		...
		if (isLoggedIn() && getLobbyButton() == null) {
			getBot().getScriptExecutor().resume();
			setFinished();
		}
		if (isDisabledMessageVisible()) {
			// NexHelper.messageQueue.push(new BannedMessage(context,
			// NexHelper.messageQueue, null));
		} else if (!getBot().isLoaded()) {
			return 1000;
		} else if (isLoggedIn() && getLobbyButton() == null) {
			getBot().getScriptExecutor().resume();
			setFinished();
